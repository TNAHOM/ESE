...
class AllInOne:
	...
	def first_choose(self):
		gen = run2.gen(self.answer)
		# print(gen[1], 'first')
		return gen, self.answer
	
	def second_choose(self):
		gen = run2.gen2(self.answer)
		# print(gen[1], 'second')
		return gen, self.answer
...
def evaluate():
		try:
			...
			if all(type(x) == list for x in parameters_list) and all(type(x) == int for x in parameters_int):
				...
				print(all_in_one.first_choose()[0][1], all_in_one.second_choose()[0][1], 'SCORE')
				score = all_in_one.first_choose()[0][1] + all_in_one.second_choose()[0][1]
				print(score, 'score')
